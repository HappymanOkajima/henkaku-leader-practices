...
『現場リーダーの技術』では、「チームプレイ」を重要な心構えとして挙げました。そこでは、「一人プレイより全員プレイ」「チームの勝利が個人の勝利」といった考え方を中心に据えていました。
...
同様に、「中庸であること」という心構えも、私の中で意味が広がりました。『現場リーダーの技術』では、「極端に走らず、バランスを取る」ことを強調していましたが、その後の経験を通じて「多様な意見を尊重しつつも、必要なときには対立を恐れない」という、より積極的な意味合いも含まれることに気づいたのです。
...
私自身、第1章で触れたプロダクトオーナーとしての失敗を、社外のコミュニティイベントで発表したことがあります。予想以上に多くの共感を得ました。同じような経験をしていた他社のエンジニアやプロダクトマネージャーから「自分だけではなかったと知って安心した」という声をいただいたのです。リーダーが失敗を隠さず、そこからの学びを語ることで、「失敗は学びの機会」という文化が生まれていきます。
...
<img width="1024" height="559" alt="image" src="https://github.com/user-attachments/assets/c3c5835a-940f-42e2-bae5-d41133eaa2ff" />
...
『現場リーダーの技術』では「現場判断の許容範囲」として、比較的限定的な権限委譲が想定されていました。しかし現代では、より広範な意思決定を現場に委ね、「意思決定の分散化」を図ることが求められています。

**3. 行動の自由度**
判断したことを実際に行動に移すための自由度も重要です。「自分で決めてもいいが、実行は許可が必要」という状態では、真の権限移譲とは言えません。

『現場リーダーの技術』では、リーダーが「メンバーを演出する」という考え方が中心で、メンバーの役割や作業範囲を明確に定義する傾向がありました。しかし現代では「実験と学習の自由度」という考え方が広がり、より柔軟な権限移譲が行われるようになっています。「失敗してもいい領域」を明確にすることで、メンバーは安心して挑戦できるようになるのです。
...
このプロジェクトでは、1年目は私がメインで具体的な内容や進め方を決めました。しかし2年目以降、志願してくれた社員たちにメンターとしてのミッションを任せ、彼女らが1年目の内容をベースに、コンテンツの改善や進め方の工夫を重ねてくれるようになりました。

「女子高生がプログラミングに興味を持ち、理工系への進学を前向きに考えるようになる」というゴールを共有した上で、具体的な講座内容や進行方法についてはメンターチームに裁量を委ねたのです。

この権限移譲の効果を実感したのは、私がコロナに罹患して講座に出られなくなったときでした。不安はありましたが、結果として彼女らは見事に講座を運営してくれました。日頃から明確なゴールを共有し、判断と行動の権限を移譲していたからこそ、私がいなくても現場が回る状態が作れていたのです。

特に印象的だったのは、あるメンターからの提案でした。「これまでの参加者100人以上が書いたレポートを網羅的に分析すれば、今後の改善に向けた傾向が見えてくるかもしれない」。彼女は自ら手を挙げ、実際にその分析を行ってくれました。その結果、参加者がどこでつまずきやすいか、何に興味を持ちやすいかといった傾向が明らかになり、今後の教材ブラッシュアップに大いに役立つ知見が得られたのです。
...
このプロジェクトでは、私は技術的な側面でサポートしましたが、アプリの詳細設計や運用方法については、実際に使う町の職員の方々に主導権を持っていただきました。彼らこそが現場のニーズを最もよく知る「専門家」だからです。

具体的には、点検項目やワークフローの設計、UIの使いやすさなどについて、町の職員の方々が主体的に意見を出し、決定していく形をとりました。私は技術的な実現可能性やベストプラクティスについてアドバイスする「支援者」という立場に徹したのです。
...
<img width="1024" height="559" alt="image" src="https://github.com/user-attachments/assets/b24e91b7-5623-4655-9d91-211f29f64131" />
...
【図表 2-3: 権限移譲の3ステップ実践サイクル】 （*注: 円形のサイクル図で3つのステップを表現。「ステップ1：「なぜ」を共有する」「ステップ2：小さな権限移譲から始める」「ステップ3：振り返りと調整を繰り返す」の3段階を円環状に配置。各ステップには実践的なヒントや具体例を箇条書きで示す。円の中心には「自律的なチーム」と記載し、サイクルが回るごとに自律性が高まることを螺旋状の矢印で表現。図の外周には「心理的安全性」と「権限移譲」のレベルが同時に高まっていく様子を示すメーターを配置。初心者向けの「明日からできる小さなアクション」を図の下部に簡潔に列挙*）
<img width="1024" height="559" alt="image" src="https://github.com/user-attachments/assets/819ed1b2-856f-4c3c-8a36-8cf24b2f2240" />

**ステップ1：「なぜ」を共有する**
権限移譲を始める前に、まず「なぜそれが必要なのか」「どのような価値を生み出すのか」を丁寧に共有しましょう。単に「これからはあなたに任せるから」では、メンバーは戸惑うばかりです。

背景や目的、期待されるアウトカムを明確に伝えることで、メンバーは「自分事」として捉えやすくなります。「この仕事はあなたが一番詳しいから」「もっと迅速に意思決定するために」など、相手に合わせた理由付けも効果的です。
...
本章で紹介した3つのステップは中長期的な取り組みですが、今日からすぐに始められる小さなアクションもあります。以下から1つでも選んで、まず試してみましょう。
...
一方、「権限移譲」は、メンバーが自ら考え、決断し、行動できる自由と責任を与えます。かつての「段取り屋」型リーダーから、現代の「環境設計者」型リーダーへの変化を象徴するものであり、組織のアジリティと個人の成長を促進します。
...
次章では、個人の学びを組織全体に循環させる方法を探ります。野中郁次郎教授が提唱したSECIモデルを意図的に実践し、「育てて返す」という新しい組織学習のかたちを発見した3年間の旅をお伝えします。
