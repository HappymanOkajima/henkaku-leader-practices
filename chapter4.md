...
<!-- 注: ダッシュボード形式の図。左上に「毎日利用率56.3%」の円グラフ、右上に部門別利用率の棒グラフ（アジャイル部門92.0%、医療システム部門28.1%など3倍の格差を視覚化）、左下に利用ツールランキング（ChatGPT 73名、Gemini 16名、GitHub Copilot 12名）、右下に「期待」と「不安」を対比した横棒グラフ（業務効率向上192名 vs 情報の正確性懸念164名、機密情報漏洩リスク148名、過度な依存89名）を配置。全体で215名回答という規模感と、組織のAI活用の現状を一目で把握できるデザイン -->
