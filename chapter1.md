...
**OODAループ**
 軍事戦略家ジョン・ボイドによって提唱されたOODAループ（Observe/観察→Orient/情勢判断→Decide/意思決定→Act/行動）は、どちらも「観察」から始まり、環境変化への適応を重視する点で共通しています。OODAが「意思決定の速さ」に焦点を当てるのに対し、アウトカムループは「顧客価値の継続的創出」を重視する点が特徴的です。
...
**アジャイル開発（スクラム）**
 私の経験では、スクラムを正しく実践できているとき、実質的にアウトカムループが回っています。スプリントレビューで顧客フィードバックを得て（検証）、レトロスペクティブで学びを言語化し（学習）、次のスプリントプランニングで仮説を立て直す。デイリースクラムでは日々の軌道修正を行う。アウトカムループは、スクラムがなぜ機能するのかを別の視点から説明したものとも言えるでしょう。
