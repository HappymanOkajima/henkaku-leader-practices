...
皆さんは、この数字を見てどう思われますか？　2019年2月28日、HIKKOSHIクラウド for Excelのアップロードサービスが終了しました。構想から約2年、これが私たちのプロジェクトの最終成績でした。ROI（投資収益率）は0.177%。約2000万円の投資に対して、わずか35,400円の売上という結果です。

しかし不思議なことに、この壮絶な失敗が私に最も重要な学びをもたらしたのです。それは「測定可能なものが必ずしも重要ではない」という真理と、「重要なものは必ずしも測定できない」という逆説でした。
![図5-1](figures/pic5-1.png)
【図表 5-1: HIKKOSHIクラウド プロジェクト概要と結果】
<!-- 注: インフォグラフィック形式のプロジェクトサマリー。左側に投資概要（総投資額2000万円、開発期間約2年、2017年2月〜2019年2月）、中央にファネル図（プレスリリース38媒体→LP485件→ログイン150件→登録7件→購入0件→売上35,400円）で激減していく様子を視覚化、右側にROI 0.177%を大きく表示。下部に「この失敗が教えてくれた2つの真理」として「測定可能なものが必ずしも重要ではない」「重要なものは必ずしも測定できない」を配置。全体的にシリアスだが学びを強調するデザイン -->
...
2013年、永和システムマネジメントは停滞気味なビジネスを再活性化させるため、新たな事業の柱を築く必要性を感じていました。新しいビジネスを創出し、全社最適化を図り、目に見える成果を出すというミッションが掲げられ、私が、部門横断型の新規事業開発リーダーを任されることになりました。
...
受託開発における成功体験が、私の中に「受託メンタルモデル」を形成していました。このモデルは、「目の前のお客様」を満足させること、プロジェクトを完遂させること、そして従業員の稼働率を最大化することを優先します。
...
**2017年11月時点の実績**は更に深刻でした。プレスリリースは38媒体に掲載され、ランディングページへのアクセスは485件、ログインページへのアクセスが150件ありました。しかし、ユーザー登録はわずか7件、請求情報の登録は5件、アップロード数は4件（最小2万円、最大30万円）。そして**購入は0件**だったのです。
...
これらのフィードバックは、私たちが作っているものが「顧客の課題」を解決していないことを明確に示していました。しかし、私はこれらの声に正面から向き合うことができなかったのです。
...
さらに深刻だったのは、私自身の心理的な罠でした。長年の受託開発経験で培われた「受託メンタルモデル」は、目の前の顧客を満足させることには長けていましたが、まだ見ぬ市場の顧客を想像することは苦手でした。そして、私には「あれは私が創った」と言いたいエゴがありました。チームメンバーに成功の証として形あるものを見せたかった。このエゴが、失敗の兆候から目を背けさせ、プロジェクトへの固執を強めたのです。
...
![図5-2](figures/pic5-2.png)
【図表 5-2: 事業計画OKRと本来設定すべきだったOKRの比較】
<!-- 注: 左右対比のレイアウト。左側「実際の事業計画」は薄い赤背景で、Objective「事業を成功させる」とKR（投資回収、ストックビジネス拡大、福井開発体制増強、人材育成、アジャイルによる実現）を列挙し、各KRに×マークと問題点（「手段が目的化」「定量目標なし」「顧客不在」）を付記。右側「本来設定すべきだったOKR」は薄い緑背景で、Objective「Excelマクロ依存業務を安全にクラウド化し生産性を解放する」とKR（有料契約10社、機密情報対策確立、コスト30%以下、ユーザーの声5件）を列挙し、各KRに○マークと「顧客価値直結」のラベル。中央に大きな矢印で「この転換が必要だった」と示す -->
...
![図5-3](figures/pic5-3.png)
【図表 5-3: HIKKOSHIクラウド 四半期ごとの警告サインと対応】
<!-- 注: 横軸に時間（2017年Q2〜2019年Q1）を配置したタイムライン。各四半期に「警告サイン」（赤信号アイコン）と「実際の対応」（グレー矢印）を対比表示。2017年Q3：警告「ニーズがない？」→対応「継続」。2017年Q4：警告「購入0件」→対応「継続」。2018年Q1：警告「機密懸念、価格懸念」→対応「大規模向けピボット」。2018年Q2-Q4：警告「変化なし」→対応「継続」。2019年Q1：サービス終了。下部に「OKRがあれば」として2017年Q3時点での「学習重視OKRへの転換」を緑の分岐矢印で示し、早期撤退・学習獲得のシナリオを提示 -->
...
Objectiveが曖昧で、KRも「学ぶ」「作る」という手段に終始しています。当初は方向性が定まりませんでした。
...
![図5-4](figures/pic5-4.png)
【図表 5-4: OKR設計チェックリストとアンチパターン】
<!-- 注: 2セクション構成の実践ガイド図。上部「5つの原則チェックリスト」はチェックボックス形式で（1）Objectiveは心を動かす北極星か（2）KRは結果であって活動ではないか（3）達成率70%程度の野心的目標か（4）KRは3-5個に絞られているか（5）四半期見直しの勇気があるか、を列挙。下部「3つのアンチパターン」は×マーク付きで（1）手段の目的化（例：アジャイル実践がO）（2）測定のための測定（例：ダッシュボード10個作成がKR）（3）現状維持OKR（例：障害月1回以下がKR）を図示。右側に「自問すべき5つの質問」を吹き出しで配置 -->

## 実践ツール

### OKR設計テンプレート

GoogleやIntelで活用されているOKRの基本フォーマットを、日本の組織向けにアレンジしたテンプレートです。

```
【OKR設計シート】

■ 対象期間：____年 Q__（__月〜__月）
...
KR1: _______________________________________________
     測定方法：_____________ 現在値：____ 目標値：____

KR2: _______________________________________________
     測定方法：_____________ 現在値：____ 目標値：____

KR3: _______________________________________________
     測定方法：_____________ 現在値：____ 目標値：____

（KRは3〜5個に絞る。達成率70%程度が理想的な野心度）
...
【中間レビュー（6週目）】
・KR1 進捗：___% 　所感：_________________________
・KR2 進捗：___% 　所感：_________________________
・KR3 進捗：___% 　所感：_________________________
・方向転換の必要性：□なし □軽微な調整 □大幅な見直し

【最終レビュー（12週目）】
・KR1 達成：___% 　学び：_________________________
・KR2 達成：___% 　学び：_________________________
・KR3 達成：___% 　学び：_________________________
・次期への示唆：________________________________
```

**活用のポイント**

1. **Objectiveは「北極星」として機能させる**
   - 毎週のチームミーティングで「これはObjectiveに近づいているか？」と問いかける
   - 判断に迷ったら、Objectiveに立ち返る

2. **KRは「結果」であって「活動」ではない**
   - NG：「ミーティングを週2回実施する」
   - OK：「意思決定スピードを50%向上させる」

3. **四半期レビューで「勇気ある撤退」を**
   - 達成困難なKRを無理に続けるより、学びを得て方向転換する
   - 「失敗」ではなく「学習」として捉える
...
- 理想的なOKR：「レガシーシステム移行に悩む企業の真の課題を発見し、解決する」
...
稼働率100%でも価値ゼロという現実があります。アジャイル開発の形式だけでは顧客は喜ばないという事実も学びました。そして何より、KPIは手段であって目的ではないという真理を痛感したのです。これらの学びは、高い授業料を払って得た、かけがえのない財産です。

### OKRが示す新しい価値観

OKRとの出会いは、目標設定に対する私の考え方を根本から変えました。

Objectiveという北極星が、チームの情熱と方向性を生み出します。Key Resultsは顧客価値の現れであるべきなのです。そして四半期ごとの見直しが、勇気ある方向転換を可能にします。
...
現在、その培われた組織能力自体が、アジャイルトレーニング、アジャイルコーチング、アジャイル開発支援、そしてスタジオツアーといったサービスとして提供されています。2000万円の投資は、単一のプロダクトとしては失敗でしたが、組織能力という形で十分なリターンを生み出したと言えるでしょう。
