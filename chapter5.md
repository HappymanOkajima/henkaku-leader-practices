...
【図表 5-1: HIKKOSHIクラウド プロジェクト概要と結果】（*注: インフォグラフィック形式のプロジェクトサマリー。左側に投資概要（総投資額2000万円、開発期間約2年、2017年2月〜2019年2月）、中央にファネル図（プレスリリース38媒体→LP485件→ログイン150件→登録7件→購入0件→売上35,400円）で激減していく様子を視覚化、右側にROI 0.177%を大きく表示。下部に「この失敗が教えてくれた2つの真理」として「測定可能なものが必ずしも重要ではない」「重要なものは必ずしも測定できない」を配置。全体的にシリアスだが学びを強調するデザイン*）
...
【図表 5-2: 事業計画OKRと本来設定すべきだったOKRの比較】（*注: 左右対比のレイアウト。左側「実際の事業計画」は薄い赤背景で、Objective「事業を成功させる」とKR（投資回収、ストックビジネス拡大、福井開発体制増強、人材育成、アジャイルによる実現）を列挙し、各KRに×マークと問題点（「手段が目的化」「定量目標なし」「顧客不在」）を付記。右側「本来設定すべきだったOKR」は薄い緑背景で、Objective「Excelマクロ依存業務を安全にクラウド化し生産性を解放する」とKR（有料契約10社、機密情報対策確立、コスト30%以下、ユーザーの声5件）を列挙し、各KRに○マークと「顧客価値直結」のラベル。中央に大きな矢印で「この転換が必要だった」と示す*）
...
【図表 5-3: HIKKOSHIクラウド 四半期ごとの警告サインと対応】（*注: 横軸に時間（2017年Q2〜2019年Q1）を配置したタイムライン。各四半期に「警告サイン」（赤信号アイコン）と「実際の対応」（グレー矢印）を対比表示。2017年Q3：警告「ニーズがない？」→対応「継続」。2017年Q4：警告「購入0件」→対応「継続」。2018年Q1：警告「機密懸念、価格懸念」→対応「大規模向けピボット」。2018年Q2-Q4：警告「変化なし」→対応「継続」。2019年Q1：サービス終了。下部に「OKRがあれば」として2017年Q3時点での「学習重視OKRへの転換」を緑の分岐矢印で示し、早期撤退・学習獲得のシナリオを提示*）
...
【図表 5-4: FDO 3年間のOKR進化と成果】（*注: 3つのカラムで3年間を比較する構成。各年度のObjective、Key Results、成果、課題を整理。41期（2021年度）：曖昧なO「新技術を幅広く獲得」→結果「バベルの塔」（方向性散漫）。42期（2022年度）：チーム重視O「アジャイルに機械学習を学ぶ」→結果「チームワーク向上」だが「技術獲得ペース低下」。43期（2023年度）：価値創出O「AIで実務価値を生み出す」→結果「就業規則Bot」「対話型ロボット」「コンペ12位」の具体的成果。下部に「OKRの成熟度」を1-5段階で視覚化（41期:2、42期:3、43期:4.5）。ピボットの判断（強化学習→自然言語処理）を転換点として強調*）
...
【図表 5-5: OKR設計チェックリストとアンチパターン】（*注: 2セクション構成の実践ガイド図。上部「5つの原則チェックリスト」はチェックボックス形式で（1）Objectiveは心を動かす北極星か（2）KRは結果であって活動ではないか（3）達成率70%程度の野心的目標か（4）KRは3-5個に絞られているか（5）四半期見直しの勇気があるか、を列挙。下部「3つのアンチパターン」は×マーク付きで（1）手段の目的化（例：アジャイル実践がO）（2）測定のための測定（例：ダッシュボード10個作成がKR）（3）現状維持OKR（例：障害月1回以下がKR）を図示。右側に「自問すべき5つの質問」を吹き出しで配置*）
